...
import { addExamGroupAction } from "@Reduxs/QuestionLibrary/action";
...
const ModalAddExamGroup = (props) => {
  const { triggerOpenModalAddExamGroup } = props;
  const [avatars, setAvatars] = useState([]);
  const [examGroupImage, setExamGroupImage] = useState(null);

  const dispatch = useDispatch();

  const schema = yup.object().shape({
    title: yup.string().required("Vui lòng nhập  tên nhóm !"),
    // description: yup.string().required("Vui lòng nhập  mô tả nhóm !"),
  });
  ...
  const onChangeAvatar = async (imageList, addUpdateIndex) => {
    ...
    setExamGroupImage(imageID);
  };
  const handleSubmitForm = (values) => {
    const callback = () => {
      triggerOpenModalAddExamGroup();
    };
    const data = {
      title: values?.title.trim(),
      description: values?.description.trim(),
      image: examGroupImage,
    };

    console.log(data)
    dispatch(addExamGroupAction(data, callback));
  };
  return (
    <ModalWrapper
      triggerModalWrapper={triggerOpenModalAddExamGroup}
      className="rc-modal-paper-add-exam-group"
      title={"Thêm nhóm"}
    >
      ...
      <Grid container>
        ...
        >
          {({
            ...
            <Div className="rc-add-exam-group-avatar">
              {imageList.length === 0 && !examGroupImage ? (
                <Div
        ...
        <Grid
          ...
          container
          xs={12}
          className="rc-modal-add-exam-group-main-form-item"
        >

            <TextField
              className="rc-modal-add-exam-group-main-form-item-text_field"
              name="title"
              label="Tên nhóm"
              inputRef={register}
              InputProps={{
...
export default ModalAddExamGroup;
